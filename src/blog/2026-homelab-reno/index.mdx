...
> [!NOTE]
> You can find my NixOS configuration at https://github.com/wongcallum/nixos.
...
called [Prism Tower](https://github.com/wongcallum/prism-tower).

I generate the service entries on the dashboard based on a JSON file generated from NixOS options,
passed into the package as an attribute override. See how I did it in [my post](/nix-package-override-flake-parts).
...
I generated the certificates with Easy-RSA, added them as sops-nix secrets, and then defined a new CA in Caddy
with the root CA certificate and key pointing to their respective secrets.
...
For each service, I defined a reverse proxy virtual host and set my previously defined CA.
...
I used the Technitium DNS Server and added an A record to the machine's Tailscale IP via the WebUI.
